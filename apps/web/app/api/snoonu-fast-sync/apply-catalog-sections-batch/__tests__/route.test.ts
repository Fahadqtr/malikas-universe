/**
 * Route tests for POST /api/snoonu-fast-sync/apply-catalog-sections-batch — owner only.
 * OPTIONS stays public (CORS). No real network.
 */
...
// Max is 50 sections / 500 products per section — unchanged. Use a small batch.
const validBody = { sections: [{ section_name: 'Hair Care', source_url: 'https://x', pages_scanned: 1, products: [] }] };
...
describe('owner', () => {
  ...
  it('requireActor called with ownerOnly; runs a mocked batch', async () => {
    createAdminMock.mockReturnValue(makeAdmin([{ data: [], error: null }, { data: { id: 1 }, error: null }]));
    const res = await POST(reqWith(validBody));
    expect(requireActorMock).toHaveBeenCalledWith(['owner']);
    expect(res.status).toBe(200);
  });

  it('candidate load failure → 500 generic; no SQL in results', async () => {
