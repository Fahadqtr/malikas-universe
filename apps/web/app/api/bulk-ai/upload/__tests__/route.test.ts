/**
 * Route tests for POST /api/bulk-ai/upload — owner or editor (ROLE_SETS.writers).
 *
 * Uploads a temp image to product-images/bulk-temp/{uuid} with the service-role
 * client, so it must authorize BEFORE formData, file read, arrayBuffer, admin
 * client, or Storage, and it must never leak raw Storage error text.
 */
...
function makeFile(overrides: Partial<{ type: string; size: number; name: string }> = {}) {
  return {
    ...
    name: overrides.name ?? 'bulk-photo.PNG',
...
describe('owner/editor happy path', () => {
  it.each([['owner'], ['editor']])('%s → 200; uploads to bulk-temp/ in product-images', async (role) => {
    ...
    expect(path).toMatch(/^bulk-temp\/.+\.(jpg|png|webp)$/);
    expect(opts).toMatchObject({ upsert: false, contentType: 'image/jpeg' });
    // response shape preserved (incl. original_filename + id correlation key)
    expect(body.data).toMatchObject({ path, size_bytes: 1234, content_type: 'image/jpeg', original_filename: 'bulk-photo.PNG' });
...
describe('existing file validation preserved (owner)', () => {
  ...
  it('bad format → 400 BAD_FORMAT', async () => {
    createAdminMock.mockReturnValue(makeAdmin({ error: null }));
    const res = await POST(req(makeFile({ type: 'application/pdf' })));
    expect(res.status).toBe(400);
