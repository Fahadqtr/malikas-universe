/**
 * Route tests for POST /api/snoonu-catalog-mapper/auto-fill-from-sections — owner only.
 */
...
describe('owner', () => {
  ...
  it('requireActor called with ownerOnly; runs a mocked fill (sections present, no products)', async () => {
    createAdminMock.mockReturnValue(makeAdmin([
      { data: [{ catalog_name_en: 'Hair Care' }], error: null }, // sections load
      { data: [], error: null },                                  // products load
    ]));
    const res = await POST(reqWith({ import_id: 1 }));
    expect(requireActorMock).toHaveBeenCalledWith(['owner']);
    expect(res.status).toBe(200);
    expect((await res.json()).data.scanned).toBe(0);
  });

  it('product select failure → 500 generic (no raw SQL)', async () => {
    createAdminMock.mockReturnValue(makeAdmin([
      { data: [{ catalog_name_en: 'Hair Care' }], error: null },
      { data: null, error: { message: 'deadlock on platform_products' } },
    ]));
    const res = await POST(reqWith({ import_id: 1 }));
  ...
  it('authorization runs before body read and admin client', async () => {
    createAdminMock.mockReturnValue(makeAdmin([{ data: [{ catalog_name_en: 'Hair Care' }], error: null }, { data: [], error: null }]));
    const rq = reqWith({ import_id: 1 });
    await POST(rq);
